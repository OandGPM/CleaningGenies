...
const DEFAULT_RATECARD = {
  ...
  catalog: [
    { id: "bed-single", label: "Strip & make bed â single", price: 7, unit: "per bed" },
    { id: "bed-double", label: "Strip & make bed â double", price: 10, unit: "per bed" },
    { id: "bed-king", label: "Strip & make bed â king/queen", price: 13, unit: "per bed" },
...
// short pages (SA103S 2024â25). Equipment is flagged separately since it's
...
const CATEGORY_TO_SA103 = {
  ...
  "Equipment": { box: "Box 23 (capital allowances)", label: "Annual Investment Allowance â not part of Box 20 total, see note" },
};
...
// UK tax year runs 6 Apr â 5 Apr
...
const ukTaxYearLabel = (startIso) => {
  const startYear = Number(startIso.slice(0, 4));
  return `${startYear}â${String(startYear + 1).slice(2)}`;
};
...
const MESSAGE_TEMPLATES = [
  { id: "late", label: "Running late", icon: Clock,
    body: "Hi {client}, it's {business} â just letting you know {cleaner} is running about {minutes} minutes behind schedule today and will arrive closer to {time}. Sorry for the short notice, and thanks for your patience!" },
  { id: "reschedule", label: "Need to change day", icon: Calendar,
    body: "Hi {client}, it's {business}. We need to move your clean originally booked for {oldDate} â would {newDate} work instead? Let us know and we'll get it locked in. Sorry for the inconvenience!" },
  { id: "holiday", label: "Team holiday / time off", icon: Sparkles,
    body: "Hi {client}, it's {business}. Just a heads up that our team will be taking some time off from {startDate} to {endDate}, so your clean on {affectedDate} will be paused for that week. We'll pick back up on your usual schedule straight after â thanks for understanding!" },
  { id: "onway", label: "On our way", icon: RouteIcon,
    body: "Hi {client}, it's {business} â {cleaner} is on the way and should be with you by around {time}. See you soon!" },
...
// ---------- postcode lookup (UK, free, no key â postcodes.io) ----------
async function geocodePostcode(postcode) {
  ...
  try {
    ...
    return { status: "error" }; // network/CORS blocked â caller should offer manual fallback
  }
}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-baseline justify-between">
        <h2 className="text-base font-bold text-stone-800">Today Â· {formatDate(today)}</h2>
      ...
      {byCleaner.map(({ cleaner, jobs: cj }) => (
        <Card key={cleaner.id} className="p-3.5">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-stone-400">Â£{cleaner.rate}/hr</span>
          </div>
                ...
                return (
                  <div key={j.id} className="text-xs bg-stone-50 rounded-lg px-2.5 py-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-mono text-stone-500 shrink-0">{j.startTime}â{addMinutes(j.startTime, mins)}</span>
                    ...
                    {client?.notes && <div className="text-amber-700 mt-1 pl-9">â­ {client.notes}</div>}
                  </div>
                );
              })}
            </div>
    ...
    <div className="space-y-3">
        ...
        <Card key={c.id} className="p-3.5 flex items-start gap-2.5" onClick={() => startEdit(c)}>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {c.blueLight && <Badge tone="gold">Blue Light â{ratecard.blueLightPct}%</Badge>}
            </div>
            <div className="text-xs text-stone-400">{[c.houseNumber, c.address, c.postcode].filter(Boolean).join(", ")}</div>
            {(c.phone || c.email) && <div className="text-xs text-stone-400">{[c.phone, c.email].filter(Boolean).join(" Â· ")}</div>}
            <div className="text-xs text-stone-500 mt-0.5">Â£{c.rate} {c.rateType}</div>
            {c.extras?.length > 0 && <div className="text-xs text-amber-700 mt-0.5">+ {c.extras.map((e) => `${e.label}${e.qty > 1 ? ` Ã${e.qty}` : ""}`).join(", ")}</div>}
      ...
      {open && (
        <Modal onClose={() => setOpen(false)} title={editing ? "Edit client" : "Add client"}>
          ...
          {geoStatus === "checking" && <p className="text-xs text-stone-400 -mt-2 mb-3">Looking up postcodeâ¦</p>}
          {geoStatus === "found" && <p className="text-xs -mt-2 mb-3" style={{ color: NAVY }}>â Location found â this client will be included in optimised routes automatically.</p>}
          {geoStatus === "notfound" && <p className="text-xs text-amber-700 -mt-2 mb-3">That postcode wasn't recognised â double check it and tap Find again.</p>}
          {geoStatus === "error" && (
          ...
          <div className="grid grid-cols-2 gap-2">
            <Field label="Rate (Â£)"><input type="number" className={inputCls} value={form.rate} onChange={(e) => setForm({ ...form, rate: e.target.value })} /></Field>
          ...
          <p className="text-xs text-stone-400 -mt-2 mb-3">Rate auto-fills from your Pricing tab when you pick a clean type â edit it here for this client if they get a custom deal. For "per hour" this is the total contracted job size (e.g. a 3-hour clean is billed as 3 hours whether 1 or 4 cleaners attend).</p>
          ...
          <label className="flex items-center gap-2 mb-3 text-sm text-stone-700">
            ...
            Blue Light card holder (â{ratecard.blueLightPct}% on the clean rate)
          </label>
              ...
              <div className="space-y-1 max-h-40 overflow-y-auto">
                {form.visitLog.map((v) => (
                  <div key={v.id} className="flex items-start justify-between bg-stone-50 rounded-lg px-2.5 py-1.5 text-xs gap-2">
                    <div><span className="font-semibold text-stone-600">{formatDate(v.date)}</span> <span className="text-stone-600">â {v.note}</span></div>
          ...
          <Field label="Extras for this client">
            <div className="space-y-1 mb-2">
              {(form.extras || []).map((ex) => (
                <div key={ex.id} className="flex items-center justify-between bg-stone-50 rounded-lg px-2.5 py-1.5 text-xs">
                  <span className="text-stone-700">{ex.label}{ex.qty > 1 ? ` Ã${ex.qty}` : ""}</span>
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-stone-600">Â£{(ex.unitPrice * ex.qty).toFixed(2)}</span>
            ...
            <div className="flex gap-1.5 mb-1.5">
              <select className={`${inputCls} flex-1`} value={catalogPick.id} onChange={(e) => setCatalogPick({ ...catalogPick, id: e.target.value })}>
                <option value="">Add from price listâ¦</option>
                {ratecard.catalog.map((item) => <option key={item.id} value={item.id}>{item.label} â Â£{item.price} ({item.unit})</option>)}
              </select>
            ...
            <div className="flex gap-1.5">
              ...
              <input type="number" className={`${inputCls} w-16`} placeholder="Â£" value={extraDraft.price} onChange={(e) => setExtraDraft({ ...extraDraft, price: e.target.value })} />
      ...
      <Card className="p-4">
        ...
        {previewMins != null && (
          <p className="text-xs -mt-1 mb-2" style={{ color: NAVY }}>
            {form.cleanerIds.length} cleaner{form.cleanerIds.length !== 1 ? "s" : ""} â about {previewMins} min on site ({form.startTime}â{addMinutes(form.startTime, previewMins)}). Each cleaner is paid for their {previewMins} min at their own rate; the client is billed for the full {form.hours || 0} hour job.
          </p>
        )}
          ...
          return (
            <Card key={j.id} className={`p-3 flex items-center gap-2.5 ${past ? "opacity-50" : ""}`}>
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-stone-400 truncate">{joinNames(team) || "No cleaners assigned"} Â· {j.hours}hr job ({mins} min on site)</div>
              </div>
        ...
        return (
          <Card key={c.id} className="p-3.5">
            ...
            <ol className="space-y-2">
              <li className="flex items-center gap-2 text-xs text-stone-400"><span className="w-5 h-5 rounded-full bg-stone-100 flex items-center justify-center font-bold shrink-0">â¢</span> Depot / start</li>
              {ordered.map((s, i) => (
                <li key={s.jobId} className="text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono text-stone-400 shrink-0">{s.startTime}â{addMinutes(s.startTime, s.mins)}</span>
                  </div>
                  {s.team.length > 0 && <div className="text-xs text-stone-500 pl-7 mt-0.5">with {joinNames(s.team)}</div>}
                  {s.notes && <div className="text-xs text-amber-700 pl-7 mt-0.5">â­ {s.notes}</div>}
                </li>
              ))}
              <li className="flex items-center gap-2 text-xs text-stone-400"><span className="w-5 h-5 rounded-full bg-stone-100 flex items-center justify-center font-bold shrink-0">â¢</span> Back to depot</li>
            </ol>
          </Card>
        );
      })}
...
function Team({ cleaners, setCleaners }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-stone-400 -mt-2">When you book a job, pick 1â4 of these cleaners to work it together â pay and on-site time are worked out automatically from the team size.</p>
      {cleaners.map((c) => {
    ...
    <div className="space-y-3">
      ...
      <Card className="p-4">
        ...
        <div className="flex gap-1.5">
          ...
          <input type="number" className={`${inputCls} w-16`} placeholder="Â£" value={newItem.price} onChange={(e) => setNewItem({ ...newItem, price: e.target.value })} />
    ...
    <div className="space-y-3">
      ...
      {policies.map((p) => (
        <Card key={p.id} className="p-3.5" onClick={() => startEdit(p)}>
          ...
          <p className="text-xs text-stone-500 whitespace-pre-wrap line-clamp-3">{p.body || "Tap to add detailsâ¦"}</p>
        </Card>
      ))}
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <Card className="p-3 text-center">
          ...
          <div className="text-sm font-bold text-stone-800">Â£{monthTotal.toFixed(2)}</div>
        </Card>
        <Card className="p-3 text-center">
      ...
      {shown.map((e) => (
        <Card key={e.id} className="p-3 flex items-center gap-2.5" onClick={() => startEdit(e)}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-xs text-stone-400">{formatDate(e.date)}{e.category === "Mileage" ? ` Â· ${e.miles} mi` : ""}{e.notes ? ` Â· ${e.notes}` : ""}</div>
          </div>
          <span className="text-sm font-bold text-stone-800 shrink-0">Â£{Number(e.amount).toFixed(2)}</span>
        </Card>
      ))}
      ...
      <p className="text-xs text-stone-400 px-1">This is a record-keeping tool, not tax software â it doesn't file anything with HMRC. For Self Assessment or Making Tax Digital, use an accountant or dedicated bookkeeping software; this just keeps your receipts and mileage organised for that.</p>
          ...
          {form.category === "Mileage" ? (
            <>
              ...
              <p className="text-xs -mt-2 mb-3" style={{ color: NAVY }}>
                {priorMiles.toLocaleString()} miles already logged this UK tax year (from 6 Apr). At HMRC rates (45p up to 10,000 miles/year, 25p after), this trip works out to <strong>Â£{(computedMileageAmount || 0).toFixed(2)}</strong>.
              </p>
            </>
          ) : (
            <Field label="Amount (Â£)"><input type="number" step="0.01" className={inputCls} value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} /></Field>
          )}
          ...
          <Field label="Receipt photo (optional)">
            {receiptPreview ? (
              ...
              <label className="flex items-center justify-center gap-2 rounded-lg border-2 border-dashed border-stone-300 py-4 text-sm text-stone-500 cursor-pointer">
                ...
                {uploading ? "Processingâ¦" : "Take or choose a photo"}
...
function Reports({ cleaners, clients, jobs, expenses, ratecard }) {
  ...
  const exportSA103 = () => downloadCSV(`sa103-summary-${ukTaxYearLabel(taxYearStart)}.csv`, [
    ...
    ["Box 20", "Total allowable expenses (Box 11â19)", box20Total.toFixed(2)],
    ["Box 21", "Net profit estimate (Box 9 â Box 20)", netProfitEstimate.toFixed(2)],
    ["Box 23 (capital allowances)", "Equipment â claim via Annual Investment Allowance, not Box 20", equipmentTotal.toFixed(2)],
  ]);
    ...
    <div className="space-y-3">
      <Card className="p-3.5 flex items-center justify-between">
        ...
        <div className="text-center">
          <div className="text-xs font-semibold text-stone-800">{formatDate(weekStart)} â {formatDate(weekEnd)}</div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <Card className="p-3 text-center">
          ...
          <div className="text-sm font-bold text-stone-800">Â£{weekRevenue.toFixed(0)}</div>
        </Card>
        <Card className="p-3 text-center">
          <div className="text-[10px] text-stone-400 uppercase font-semibold">Payroll</div>
          <div className="text-sm font-bold text-stone-800">Â£{weekPayroll.toFixed(0)}</div>
        </Card>
        <Card className="p-3 text-center">
          <div className="text-[10px] text-stone-400 uppercase font-semibold">Expenses</div>
          <div className="text-sm font-bold text-stone-800">Â£{weekExpensesTotal.toFixed(0)}</div>
        </Card>
      </div>
      ...
      <Card className="p-4 text-white" style={{ background: NAVY }}>
        ...
        <div className="text-2xl font-bold">Â£{net.toFixed(2)}</div>
      ...
      {payRows.map(({ cleaner, jobCount, hours, pay }) => (
        <Card key={cleaner.id} className="p-3.5">
          <div className="flex items-center justify-between">
            ...
            <span className="text-base font-bold" style={{ color: NAVY }}>Â£{pay.toFixed(2)}</span>
          </div>
          <div className="text-xs text-stone-400 mt-0.5">{jobCount} job{jobCount !== 1 ? "s" : ""} Â· {hours.toFixed(2)} hrs on site Â· Â£{cleaner.rate}/hr</div>
        </Card>
      ))}

      <Card className="p-4">
        <h2 className="text-sm font-bold text-stone-800 mb-1">Self Assessment summary (SA103S)</h2>
        <p className="text-xs text-stone-400 mb-3">Maps your logged expenses to the current short self-employment pages, box by box, so you can copy the figures straight across. This organises your records â it isn't tax advice and doesn't check anything with HMRC, so it's worth a quick sense-check (or an accountant) before you file, especially if turnover is near Â£90,000 or you have capital equipment.</p>
        ...
        <div className="space-y-1.5 text-xs">
          <div className="flex justify-between bg-stone-50 rounded-lg px-2.5 py-1.5">
            <span><strong>Box 9</strong> â Turnover (estimated)</span><span className="font-semibold">Â£{turnover.toFixed(2)}</span>
          </div>
          {boxTotals.map((b) => (
            <div key={b.box} className="flex justify-between px-2.5 py-1">
              <span>{b.box} â {b.label}</span><span>Â£{b.total.toFixed(2)}</span>
            </div>
          ))}
          <div className="flex justify-between bg-stone-50 rounded-lg px-2.5 py-1.5 font-semibold">
            <span>Box 20 â Total allowable expenses</span><span>Â£{box20Total.toFixed(2)}</span>
          </div>
          <div className="flex justify-between rounded-lg px-2.5 py-1.5 font-bold" style={{ background: NAVY_TINT, color: NAVY }}>
            <span>Box 21 â Net profit (estimate)</span><span>Â£{netProfitEstimate.toFixed(2)}</span>
          </div>
          {equipmentTotal > 0 && (
            <div className="flex justify-between text-amber-700 px-2.5 py-1">
              <span>Equipment: Â£{equipmentTotal.toFixed(2)} â usually goes under capital allowances (Box 23, Annual Investment Allowance), not Box 20</span>
            </div>
          )}
        </div>
...
function Invoices({ clients, jobs, business, setBusiness, ratecard }) {
  ...
  const printInvoice = () => {
    ...
    const cleanRows = occurrences.map((o) =>
      `<tr><td style="padding:6px 8px;border-bottom:1px solid #eee">${formatDate(o.date)}</td><td style="padding:6px 8px;border-bottom:1px solid #eee">${o.job.startTime}</td><td style="padding:6px 8px;border-bottom:1px solid #eee">${o.job.hours} hr clean</td><td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right">Â£${o.amount.toFixed(2)}</td></tr>`
    ).join("");
    const discountRow = discountAmount > 0 ? `<tr><td colspan="3" style="padding:6px 8px;border-bottom:1px solid #eee;color:${NAVY}">Blue Light discount (${ratecard.blueLightPct}%)</td><td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right;color:${NAVY}">âÂ£${discountAmount.toFixed(2)}</td></tr>` : "";
    const extraRows = (client?.extras || []).filter((e) => selectedExtras.includes(e.id)).map((e) =>
      `<tr><td colspan="3" style="padding:6px 8px;border-bottom:1px solid #eee">${e.label}${e.qty > 1 ? ` Ã${e.qty}` : ""}</td><td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right">Â£${(e.unitPrice * e.qty).toFixed(2)}</td></tr>`
    ).join("");
    win.document.write(`
      ...
      <body style="font-family:Georgia,serif;color:#222;max-width:600px;margin:40px auto;">
        ...
        <p style="color:#888;margin-top:4px;font-family:Arial,sans-serif;">${[business.houseNumber, business.address].filter(Boolean).join(" ")}${business.postcode ? ", " + business.postcode : ""} ${business.phone ? " Â· " + business.phone : ""} ${business.email ? " Â· " + business.email : ""}</p>
        ...
        <p style="color:#888;font-size:13px;font-family:Arial,sans-serif;">Period: ${formatDate(periodStart)} â ${formatDate(periodEnd)}</p>
        ...
        <h3 style="text-align:right;margin-top:16px;color:${NAVY};">Total due: Â£${total.toFixed(2)}</h3>
  ...
  return (
    <div className="space-y-3">
      <Card className="p-4">
        ...
        {depotGeoStatus === "checking" && <p className="text-xs text-stone-400 -mt-1 mb-3">Looking up postcodeâ¦</p>}
        {depotGeoStatus === "found" && <p className="text-xs -mt-1 mb-3" style={{ color: NAVY }}>â Location found â used as the start/end point for optimised routes.</p>}
        {depotGeoStatus === "notfound" && <p className="text-xs text-amber-700 -mt-1 mb-3">That postcode wasn't recognised â double check it and tap Find again.</p>}
          ...
          <div className="mt-2 border-t border-stone-100 pt-3">
              ...
              <div className="space-y-1 mb-2">
                {occurrences.map((o, i) => (
                  <div key={i} className="flex justify-between text-xs text-stone-500">
                    <span>{formatDate(o.date)} Â· {o.job.startTime} Â· {o.job.hours}hr clean</span><span>Â£{o.amount.toFixed(2)}</span>
                  </div>
                ))}
              </div>
            )}
            {client.blueLight && discountAmount > 0 && (
              <div className="flex justify-between text-xs font-semibold mb-2" style={{ color: NAVY }}>
                <span>Blue Light discount ({ratecard.blueLightPct}%)</span><span>âÂ£{discountAmount.toFixed(2)}</span>
              </div>
            )}
              ...
              <div className="mb-2 space-y-1">
                ...
                {client.extras.map((e) => (
                  <label key={e.id} className="flex items-center justify-between text-xs bg-stone-50 rounded-lg px-2.5 py-1.5">
                    <span className="flex items-center gap-1.5"><input type="checkbox" checked={selectedExtras.includes(e.id)} onChange={() => toggleExtra(e.id)} />{e.label}{e.qty > 1 ? ` Ã${e.qty}` : ""}</span>
                    <span>Â£{(e.unitPrice * e.qty).toFixed(2)}</span>
                  </label>
                ))}
              </div>
            )}
          ...
          <Field key={p} label={niceLabel(p)}>
            {p === "client" ? (
              <select className={inputCls} value={values[p] || ""} onChange={(e) => setValues({ ...values, [p]: e.target.value })}>
                <option value="">Select clientâ¦</option>{clients.map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
              </select>
            ) : (
